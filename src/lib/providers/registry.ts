...
export function providerHasKind(provider: ProviderInstance, kind: ProviderKind) {
...
export function providerSupportsCapability(
  ...
  return providerHasKind(provider, kind) && providerHasCapability(provider, capability)
}
...
function getProviderPluginForInstance(provider: ProviderInstance) {
  return providerPlugins.find((plugin) => plugin.type === provider.type) ?? null
}
